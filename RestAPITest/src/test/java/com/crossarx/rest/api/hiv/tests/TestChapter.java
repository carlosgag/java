...
public class TestChapter {
	...
	public void testGetChapter() {
		try {
			Assert.assertTrue(getChapter().getId() == courseId);
	...
	@Test
	public void testGetChapterQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestChapterQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestChapterQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("chapter");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{5532[36223]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testChapterImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/chapter/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
	...
	private Chapter getChapter()
		...
		String operation = "record/chapter/" + this.id + "?authhash=" + loginResponse.getAuthHash();
		Response response = Utils.getClient().doGet(operation);
		String result = response.readEntity(String.class);
		System.out.println(result);
		JsonNode rootNode = Utils.getMapper().readTree(result);
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getChapterFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getChapterFromJson(JsonNode dataNode){
		System.out.println(dataNode);
