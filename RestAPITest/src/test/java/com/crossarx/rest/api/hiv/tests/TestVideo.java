...
public class TestVideo {
	...
	public void testGetVideo() {
		try {
			Assert.assertTrue(getVideo().getId() == this.id);
	...
	@Test
	public void testGetVideoQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestVideoQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestVideoQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("Video");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{8718<5847[65672]|5847[39443]>5534|5534[36229]>5533|5533[36226]>5532|5532[36223]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testVideoImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/Video/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
			InputStream in = (InputStream) response.readEntity(InputStream.class);
			//String path = getVideo(id).getImage();
			String path = "";
	...
	private Chapter getVideo()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getVideoFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getVideoFromJson(JsonNode dataNode){
		System.out.println(dataNode);
