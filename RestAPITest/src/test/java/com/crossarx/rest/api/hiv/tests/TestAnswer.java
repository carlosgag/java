...
public class TestAnswer {
	...
	public void testGetAnswer() {
		try {
			Assert.assertTrue(getAnswer().getId() == this.id);
	...
	@Test
	public void testGetAnswerQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestAnswerQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestAnswerQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("Answer");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{5538[36241]>5536|5536[36260]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testAnswerImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/Answer/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
			InputStream in = (InputStream) response.readEntity(InputStream.class);
			//String path = getAnswer(id).getImage();
			String path = "";
	...
	private Chapter getAnswer()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getAnswerFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getAnswerFromJson(JsonNode dataNode){
		System.out.println(dataNode);
