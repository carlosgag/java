...
public class TestMedia {
	...
	public void testGetMedia() {
		try {
			Assert.assertTrue(getMedia().getId() == this.id);
	...
	@Test
	public void testGetMediaQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestMediaQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestMediaQueryOK() {
...
//		sortSpecs.add(spec);
	...
	//@Test
	public void testMediaImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/Media/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
			InputStream in = (InputStream) response.readEntity(InputStream.class);
			//String path = getMedia(id).getImage();
			String path = "";
	...
	private Chapter getMedia()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getMediaFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getMediaFromJson(JsonNode dataNode){
		System.out.println(dataNode);
