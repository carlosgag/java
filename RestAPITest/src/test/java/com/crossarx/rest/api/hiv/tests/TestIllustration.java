...
public class TestIllustration {
	...
	@Test
	public void testIllustrationOutput() {
		try {
			System.out.println(getIllustration());
	...
	public void testGetIllustration() {
		try {
			Assert.assertTrue(getIllustration().getId() == this.id);
	...
	@Test
	public void testGetIllustrationQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestIllustrationQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestIllustrationQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("illustration");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{5847[39443]>5534|5534[36229]>5533|5533[36226]>5532|5532[36223]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testIllustrationImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/illustration/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
	...
	private Chapter getIllustration()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getIllustrationFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getIllustrationFromJson(JsonNode dataNode){
		System.out.println(dataNode);
