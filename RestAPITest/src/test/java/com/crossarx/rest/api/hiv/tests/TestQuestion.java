...
public class TestQuestion {
	...
	public void testGetQuestion() {
		try {
			Assert.assertTrue(getQuestion().getId() == this.id);
	...
	@Test
	public void testGetQuestionQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestQuestionQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestQuestionQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("Question");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{5536[36260]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testQuestionImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/Question/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
			InputStream in = (InputStream) response.readEntity(InputStream.class);
			//String path = getQuestion(id).getImage();
			String path = "";
	...
	private Chapter getQuestion()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getQuestionFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getQuestionFromJson(JsonNode dataNode){
		System.out.println(dataNode);
