...
public class TestPage {
	...
	private final Integer id = 34348;
	...
	public void testGetPage() {
		try {
			Assert.assertTrue(getPage().getId() == this.id);
	...
	@Test
	public void testGetPageQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestPageQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestPageQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("page");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{5534[36229]>5533|5533[36226]>5532|5532[36223]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testPageImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/page/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
			InputStream in = (InputStream) response.readEntity(InputStream.class);
			//String path = getPage(id).getImage();
			String path = "";
	...
	private Chapter getPage()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getPageFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getPageFromJson(JsonNode dataNode){
		System.out.println(dataNode);
