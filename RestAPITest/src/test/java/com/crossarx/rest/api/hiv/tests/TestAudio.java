...
public class TestAudio {
	...
	public void testGetAudio() {
		try {
			Assert.assertTrue(getAudio().getId() == this.id);
	...
	@Test
	public void testGetAudioQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestAudioQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestAudioQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("Audio");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{11355<5847[97526]|5847[39443]>5534|5534[36229]>5533|5533[36226]>5532|5532[36223]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testAudioImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/Audio/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
			InputStream in = (InputStream) response.readEntity(InputStream.class);
			//String path = getAudio(id).getImage();
			String path = "";
	...
	private Chapter getAudio()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getAudioFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getAudioFromJson(JsonNode dataNode){
		System.out.println(dataNode);
