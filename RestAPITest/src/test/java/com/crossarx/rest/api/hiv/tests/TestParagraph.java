...
import com.crossarx.rest.api.hiv.entities.Course;
...
public class TestParagraph {
	...
	public void testGetParagraph() {
		try {
			Assert.assertTrue(getParagraph().getId() == this.id);
	...
	@Test
	public void testGetParagraphQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestParagraphQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestParagraphQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("paragraph");

		FilterNode filter = new FilterNode();
		filter.setOperator("and");
		List<Filter> children = new ArrayList<>();
		FilterLeaf leaf1 = new FilterLeaf();
		leaf1.setField("82736{5533[36226]>5532|5532[36223]>5531|5531<10260[82737]}");
	...
	//@Test
	public void testParagraphImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/paragraph/" + this.id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
			InputStream in = (InputStream) response.readEntity(InputStream.class);
			//String path = getParagraph(id).getImage();
			String path = "";
	...
	private Chapter getParagraph()
		...
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getParagraphFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Chapter getParagraphFromJson(JsonNode dataNode){
		System.out.println(dataNode);
