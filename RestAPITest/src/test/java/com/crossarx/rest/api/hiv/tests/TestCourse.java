...
public class TestCourse {
	...
	public void testGetCourse() {
		try {
			Assert.assertTrue(getCourse().getId() == id);
	...
	@Test
	public void testGetCourseQueryOK() {
		// /engine/api/VERSION/query?querydef={}&authhash=<hash>
		try {
			Query query = makeTestCourseQueryOK();
	 ...
	 * @return
	 */
	private Query makeTestCourseQueryOK() {
...
//		sortSpecs.add(spec);
		...
		queryRequest.setClassName("course");
	...
	//@Test
	public void testCourseImageDownload() {
		// URI: /engine/api/<version>/download/<class>/<id>/Image
		String operation = "download/course/" + id + "/Image?authhash=" + loginResponse.getAuthHash();
		try {
			Response response = Utils.getClient().doGet(operation);
	...
	private Course getCourse()
		...
		String operation = "record/course/" + id + "?authhash=" + loginResponse.getAuthHash();
		Response response = Utils.getClient().doGet(operation);
		String result = response.readEntity(String.class);
		JsonNode rootNode = Utils.getMapper().readTree(result);
		JsonNode recordNode = rootNode.path("record");
		JsonNode dataNode = recordNode.path("data");
		return getCourseFromJson(dataNode);
	}
	 ...
	 * @return
	 */
	private Course getCourseFromJson(JsonNode dataNode){
		System.out.println(dataNode);
